...
const ElasticMesh = ({
  ...
  /* Upstream assigned this during render, which React 19 flags as touching a
     ref on the render path. Syncing in an effect that runs after every render
     keeps the animation loop reading current props without that. */
  ...
  useEffect(() => {
    ...
    // The loop starts before the sync effect above has run on first mount.
    if (!propsRef.current.stiffness) {
      propsRef.current = {
    ...
    try {
      ...
      // No WebGL — the still image underneath stands in for the effect.
      return undefined;
    }
